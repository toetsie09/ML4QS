...
class CreateDataset:
    ...
    def add_numerical_dataset(self, file, timestamp_col, value_cols, aggregation='avg', prefix=''):
        ...
        # tdelta = pd.to_timedelta(dataset["Milliseconds"] % 1000, unit="ms")
        # dataset[timestamp_col] = dataset[timestamp_col] + tdelta

        # print(dataset[timestamp_col].head())

        # Create a table based on the times found in the datasets
        ...
        for i in range(0, len(self.data_table.index)):
            ...
            # print('staring index: ', self.data_table.index[i])
            # print('end index: ', self.data_table.index[i] +
            #                            timedelta(milliseconds=self.granularity))
            len_list.append(len(relevant_rows))
        ...
        # dataset.columns = [prefix + x if 'Time' not in x else x for x in col_renames]
        # print(dataset.columns)
    ...
    def add_event_dataset(self, file, start_timestamp_col, end_timestamp_col, value_col, aggregation='sum'):
        ...
        # Clean the event values in the datasets
        dataset[value_col] = dataset[value_col].apply(self.clean_name)
        event_values = dataset[value_col].unique()

        # Add columns for all possible values (or create a new datasets if empty), set the default to 0 occurrences
        if self.data_table is None:
        ...
        for i in range(0, len(dataset.index)):
            # identify the time points of the row in our datasets and the value
