...
def main():
    ...
    DATASET_FNAME = 'chapter3_result_1000.csv'
    ...
    attributes = ['acc_phone_X', 'acc_phone_Y', 'acc_phone_Z']
    # attributes = ['gyr_phone_x', 'gyr_phone_y', 'gyr_phone_z']
    ...
    if FLAGS.mode == 'kmeans':
        ...
        for k in k_values:
            ...
            dataset_cluster = clusteringNH.k_means_over_instances(copy.deepcopy(
                dataset), attributes, k, 'default', 20, 10)
        ...
        dataset = clusteringNH.k_means_over_instances(copy.deepcopy(dataset), attributes, k,
                                                      'default', 50, 50)
        DataViz.plot_clusters_3d(dataset, attributes, 'cluster', ['label'])
        DataViz.plot_silhouette(dataset, 'cluster', 'silhouette')
        util.print_latex_statistics_clusters(
            dataset, 'cluster', attributes, 'label')
    ...
    if FLAGS.mode == 'kmediods':
        ...
        for k in k_values:
            ...
            dataset_cluster = clusteringNH.k_medoids_over_instances(copy.deepcopy(
                dataset), attributes, k, 'default', 20, n_inits=10)
            silhouette_score = dataset_cluster['silhouette'].mean()
            print(f'silhouette = {silhouette_score}')
            silhouette_values.append(silhouette_score)
        ...
        dataset_kmed = clusteringNH.k_medoids_over_instances(copy.deepcopy(dataset), attributes, k, 'default', 20, n_inits=50)
        DataViz.plot_clusters_3d(dataset_kmed, attributes, 'cluster', ['label'])
        DataViz.plot_silhouette(dataset_kmed, 'cluster', 'silhouette')
        util.print_latex_statistics_clusters(dataset_kmed, 'cluster', attributes, 'label')
    ...
    if FLAGS.mode == 'agglomerative':
        ...
        for k in k_values:
            print(f'k = {k}')
            dataset, l = clusteringH.agglomerative_over_instances(dataset, attributes, k, 'euclidean', use_prev_linkage=True,
                                                                  link_function='ward')
        ...
        k = k_values[np.argmax(silhouette_values)]
        print(f'Highest agglomerative silhouette score: k = {k}')

    if FLAGS.mode == 'final':
        # And we select the outcome dataset of the knn clustering....
        clusteringNH = NonHierarchicalClustering()

        dataset = clusteringNH.k_means_over_instances(dataset, attributes, FLAGS.k,
                                                      'default', 50, 50)
        DataViz.plot_clusters_3d(dataset, attributes, 'cluster', ['label'])
        DataViz.plot_silhouette(dataset, 'cluster', 'silhouette')
        util.print_latex_statistics_clusters(
